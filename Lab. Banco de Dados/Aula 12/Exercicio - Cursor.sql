...
No atributo NR_LINHA_ARQUIV, há um número que referencia a linha de incidência do endereço na tabela endereço.
Por exemplo: 
...
Portanto, o NR_LINHA_ARQUIV (1) referencia o registro do endereço da Rua A e o NR_LINHA_ARQUIV (2) referencia o endereço da Rua B.
...
EXEC sp_atualizaenvio

CREATE PROCEDURE sp_atualizaenvio
AS 
DECLARE @CPF varchar(20),
	...
	DECLARE c_envio CURSOR FOR SELECT CPF, NR_LINHA_ARQUIV FROM envio order by CPF, NR_LINHA_ARQUIV asc
	OPEN c_envio
	FETCH NEXT FROM c_envio INTO @CPF, @linha

	DECLARE c_endereco CURSOR FOR SELECT ENDEREÇO, PORTA, COMPLEMENTO, BAIRRO, CEP, CIDADE, UF FROM endereço order by CPF asc
	OPEN c_endereco
	FETCH NEXT FROM c_endereco INTO @NM_ENDERECO, @NR_ENDERECO, @NM_COMPLEMENTO, @NM_BAIRRO, @NR_CEP, @NM_CIDADE, @NM_UF

	WHILE @@FETCH_STATUS = 0
	BEGIN
	
		UPDATE envio
		SET NM_ENDERECO = @NM_ENDERECO,
