...
insert into viagem(idCarro, distanciaPercorrida, data) values
(1006,97,'2016-05-01'), 
...
SELECT c.marca, c.modelo, SUM(v.distanciaPercorrida) AS distancia_Percorrida
FROM empresa e, carro c, viagem v
WHERE e.id = c.idEmpresa
	AND c.id = v.idCarro
	AND e.nome = 'Empresa 1'
GROUP BY c.id, c.marca, c.modelo 
ORDER BY distancia_Percorrida
...
FROM viagem v RIGHT OUTER JOIN carro c
ON c.id = v.idCarro 
...
SELECT e.nome AS nome_empresa, c.marca, c.modelo, COUNT(c.id) AS viagens_feitas
FROM empresa e, carro c, viagem v
WHERE e.id = c.idEmpresa
	AND c.id = v.idCarro
GROUP BY c.id, e.nome, c.marca, c.modelo 
ORDER BY e.nome ASC, viagens_feitas DESC 
...
WHERE e.id = c.idEmpresa
	AND c.id = v.idCarro
GROUP BY c.id, e.nome, c.marca, c.modelo, v.distanciaPercorrida
ORDER BY e.nome ASC